...
const ProductSort = (props: ProductType) => {
    ...
    return (
        <div className="d-flex w-100 gap-3 h-25 mb-3 bg-white">
            <div className="w-25 border-1 border fd-hover-border-primary d-flex align-items-center">
                <Link to={"/product/" + props.id}><img src={(props.img)} alt={props.name} className="w-100 h-[100]" /></Link>
            </div>
            <div className="w-75">
                <Link to={"/product/" + props.id} style={{
                    display: "-webkit-box",
                    WebkitLineClamp: 1,
                ...
                <div className="d-flex ">
                    ...
                    <span className="align-self-end " style={{ textDecoration: "line-through", color: "black" }}>₹{props.old_price}</span>
                </div>
            </div>
        </div>
    )
...
// const ProductSort1 = (props : ProductType) => {
