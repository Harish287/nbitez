      ...
      {!isLoading && !isError ?
        slides['data'].map((slide: Slide) => {
          return <div className='f-slide position-relative shadow h-100' key={slide.id}>
            ...
            <div className="f-slide-text position-absolute text-start" style={{ width: '50%', top: '80%', left: '73%' }}>
              <div>
                {/* <h1 className='fw-bold'>{slide.text}</h1> */}
                {/* <h5 className='my-3 fw-bold'>Free Shipping on all Your Order</h5> */}
                <Link to={RoutePaths.shop} className="fd-btn col-12 col-lg-6 text-center p-3 opacity-75">SHOP NOW <i className="bi bi-arrow-right"></i></Link>
              </div>
            </div>
          </div>
        }) :
