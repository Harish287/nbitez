...
const Shop = () => {
  ...
  const { isLoading, data: categoryList, isError } = useGetAllCategoriesQuery("api/category");
  const [categoryId, setCategoryId] = useState<number | null>(null);

  const [type, setType] = useState('grid');
  console.log("gh", categoryId)
    ...
    <>
      ...
      <div className="shop d-flex flex-wrap gap-3 my-3 text-black p-3 p-lg-5">
        <div className="shop-section col-12 col-lg-8">
          ...
          <PopularProducts type={type} categoryId={categoryId} />
        </div>
                ...
                <div className="category-list d-flex flex-column gap-4 py-2 px-3">
                  {
                    ...
                    //   ? <Spinner />
                    //   : isSuccess
                    //     ? productsList['data'].filter((e: any) => e["categorie_id "] === "16").map((product: ProductType) => <ProductCart {...product} type={type} key={product.id} />)
                    //     : null;
                    categoryList['data'].map((category: CategoryType) => <Category category={category} key={category.id} onClick={() => setCategoryId(Number(category?.id))} categoryId={categoryId} />)
                  }
                </div> :
                <Spinner />
              }
